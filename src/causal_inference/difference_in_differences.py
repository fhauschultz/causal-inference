    ...
    def fit(self):
        ...
        for var in self.value_vars:
            table, model = cumulative_treatment_effects(
                self.data, self.units, "days_since_experiment_start", self.treatment_col, treatment_start_date=0, treatment_end_date=self.experiment_duration_days, covariates=self.covariates, outcome_col=var, alpha=self.confidence_level, cov_type=self.cov_type
            )
