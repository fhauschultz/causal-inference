...
def test_estimates():
    ...
    # fit model
    did_model = EventStudy(test_data, ["id"], "time", "treatment", ["y"]).fit(significance_level=0.1)
...
def test_conf_int_robust():
    ...
    # fit model
    model = EventStudy(test_data, ["id"], "time", "treatment", ["y"]).fit(significance_level=0.1)
