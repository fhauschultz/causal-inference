...
def get_inference_data(data, time_col, unit_col, value_col, treated_unit):
    ...
    """
    full_data_pivot = data.pivot(
        index=time_col, columns=unit_col, values=value_col)
    treated_values_full = full_data_pivot[treated_unit].dropna()
    donor_units = [
        unit for unit in full_data_pivot.columns if unit != treated_unit]
    donor_values_full = full_data_pivot[donor_units]

    # Align time periods for full outcomes
    common_time_full = treated_values_full.index.intersection(
        donor_values_full.index)
    x_outcome_treated = treated_values_full.loc[common_time_full].values.reshape(
        1, -1)
...
def get_training_data(data, time_col, unit_col, value_col, treated_unit, experiment_date, training_end_date=None, covariates=None):
    ...
    if training_end_date:
        pre_treatment_data = pre_treatment_data[pre_treatment_data[time_col]
                                                < training_end_date]

    # Pivot data for training
    data_pivot = pre_treatment_data.pivot(
        index=time_col, columns=unit_col, values=value_col)
    ...
    if covariates:
        for covariate in covariates:
            covar_data = pre_treatment_data.pivot(
                index=time_col, columns=unit_col, values=covariate)
            treated_covar = covar_data[treated_unit].dropna()
            donor_covar = covar_data[donor_units]

            # Align time periods
            common_time_covar = treated_covar.index.intersection(
                donor_covar.index)
...
def prepare_data(data, time_col, unit_col, value_col, treated_unit, experiment_date, training_end_date=None, covariates=None):
    ...
    """
    x_train_treated, x_train_donor = get_training_data(
        data, time_col, unit_col, value_col, treated_unit, experiment_date, training_end_date, covariates)
    x_outcome_treated, x_outcome_donor, donor_units = get_inference_data(
        data, time_col, unit_col, value_col, treated_unit)
...
def prune_units_for_se_computation(placebo_effects, experiment_date, tolerance_pct):
    good_fit_units = get_good_fit_units(placebo_effects,
                                        index_threshold=experiment_date,
...
class SyntheticControl:
    ...
    def __init__(self, data,
        ...
        self.data = filter_donor_units(
            data[self.cols].copy(deep=False), treated_unit, unit_col)
    ...
    def _get_treated_values(self, treated_unit):
        ...
        outcome_data = self.data[[
            self.unit_col, self.time_col, self.value_col]]
    ...
    def _fit_model(self, treated_unit, experiment_date, training_end_date=None):
        ...
        (x_train_treated,
         ...
         x_outcome_treated,
         x_outcome_donor,
         donor_units) = prepare_data(self.data,
                                     self.time_col,
                                     self.unit_col,
                                     self.value_col,
                                     treated_unit,
        ...
        if hasattr(self.model, 'coef_'):
            ...
            print(
                "Warning: Model must have coef_ or feature_importances_ attribute to extract weights")

        synthetic_control = self.model.predict(x_outcome_donor)

        weights_df = pd.DataFrame(weights, index=donor_units, columns=[
                                  'weight']).sort_values('weight', ascending=False)
    ...
    def fit(self, treated_unit=None, experiment_date=None, training_end_date=None, calculate_se=True, significance_level=None, prune_data_for_se_computation=True):
        ...
        self.synthetic_control, self.weights = self._fit_model(
            treated_unit, experiment_date, training_end_date)

        if calculate_se:
            self._calculate_standard_errors(
                experiment_date, significance_level, prune_data_for_se_computation)
    ...
    def _calculate_standard_errors(self, experiment_date, significance_level, prune_data):
        ...
        donor_units = [
            unit for unit in self.data[self.unit_col].unique() if unit != self.treated_unit]
        placebo_effects = []
        for donor in donor_units:
            synthetic_control, _ = self._fit_model(donor, experiment_date)
            placebo_impact = self._get_treated_values(
                donor) - synthetic_control
            placebo_effects.append(placebo_impact)

        placebo_effects = collect_series_to_dataframe(placebo_effects)

        if prune_data:
            placebo_effects = prune_units_for_se_computation(
                placebo_effects, self.experiment_date, tolerance_pre_treatment_pruning_pct)

        placebo_effects_np = np.array(placebo_effects)
        upper_bound = np.percentile(
            placebo_effects_np, 100 - significance_level / 2, axis=1)
        lower_bound = np.percentile(
            placebo_effects_np, significance_level / 2, axis=1)
        self.se = pd.DataFrame({
            'Upper Bound': upper_bound,
            'Lower Bound': lower_bound
        }, index=placebo_effects.index)
    ...
    def plot_histogram(self):
        ...
        plt.axvline(self.summary()['Effect'],
                    c='black', label='Treatment Effect')
        plt.legend()
        plt.show()
    ...
    def plot(self):
        ...
        if self.synthetic_control is None:
            raise ValueError(
                "Synthetic control has not been computed. Please run the fit method first.")
        impact = self._get_results()

        plt.figure(figsize=(10, 6))
        plt.title(self.value_col)
        plt.plot(impact.index, impact['Treated'], label='Treated Unit',
                 linestyle='-', color='blue', lw=3, alpha=0.8)
        plt.plot(impact.index, impact['Synthetic Control'],
                 label='Synthetic Control', linestyle='--', color='black', lw=3, alpha=0.8)
        if self.se_computed:
            plt.fill_between(impact.index, impact['Synthetic Control'] + impact['Lower Bound'],
                             impact['Synthetic Control'] + impact['Upper Bound'], color='grey', alpha=0.3, label='Confidence Band')
        plt.axvline(x=self.experiment_date, color='gray',
                    linestyle=':', label='Experiment Date', lw=2)

        if self.training_end_date:
            plt.axvline(x=self.training_end_date, color='magenta',
                        linestyle='--', label='Training End Date', alpha=0.5, lw=2)
    ...
    def plot_effect(self):
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(impact.index, impact['Effect'], label='Treated Unit',
                 linestyle='-', color='blue', lw=3, alpha=0.8)
        if self.se_computed:
            plt.fill_between(confidence_band.index, confidence_band['Lower Bound'],
                             confidence_band['Upper Bound'], color='grey', alpha=0.3, label='Confidence Band')
        plt.axvline(x=self.experiment_date, color='gray',
                    linestyle=':', label='Experiment Date', lw=2)
